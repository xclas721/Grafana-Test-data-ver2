...
const loginAndBack = async (payload: { username: string; password: string }) => {
  ...
  try {
    ...
    authStore.setToken(result.token ?? null)
    authStore.setUser({ name: payload.username })
...
const submitLogin = async () => {
  await loginAndBack({ username: username.value.trim(), password: password.value })
}
</script>
    ...
    >
      <div class="space-y-6 p-1 sm:p-2">
        <div class="text-center space-y-2">
          <h1 class="text-2xl sm:text-3xl font-bold tracking-tight">{{ t('ui.login.title') }}</h1>
          <p class="text-sm sm:text-base text-base-content/60">{{ t('ui.login.description') }}</p>
        </div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
          <Button variant="primary" class="w-full" :disabled="isSubmitting" @click="submitLogin">
            {{ isSubmitting ? t('ui.login.submitting') : t('ui.login.submitButton') }}
          </Button>
